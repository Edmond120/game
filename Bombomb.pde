class Bombomb extends battleMode{
  void _setup(){
    ...
    unit a = new PlayerBomber(this,0.5,0.5,0.20,0.5);
    players.add(a);
...
class PlayerBomber extends player{
  float speed = 0.05 * scale;
  PlayerBomber(entity parent, battleMode field, float xcor, float ycor, float size, float displaySize){
    this.parent = parent;
    this.field = field;
    this.xcor = xcor;
    this.ycor = ycor;
  ...
  PlayerBomber(battleMode field, float xcor, float ycor, float size, float displaySize){
    this.field = field;
  ...
  void move(){
   if(keys[keyW] || codedKeys[cKeyUP]){
     ycor -= speed;
     face[0] = -1;
   }
   if(keys[keyS] || codedKeys[cKeyDOWN]){
     ycor += speed;
     face[0] = 1;
   }
   if(keys[keyA] || codedKeys[cKeyLEFT]){
     xcor -= speed;
     face[1] = -1;
   }
   if(keys[keyD] || codedKeys[cKeyRIGHT]){
     xcor += speed;
     face[1] = 1;
   }
  }
